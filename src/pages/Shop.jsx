...
function Shop() {
  ...
  return (
    <section className="flex flex-col md:flex-row max-w-7xl mx-auto gap-4 px-2 md:px-4 py-4">
      ...
      <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-2 md:p-4">
        {filteredData.map(item => (
          <div
            key={item.id}
            className="border rounded-xl shadow hover:shadow-lg transition duration-200 p-3 bg-white flex flex-col cursor-pointer group"
          >
            <img
              src={item.image}
              alt={item.name}
              onClick={() => navigate(`/product/${item.id}`)}
              className="w-full h-44 md:h-48 object-cover mb-3 rounded-lg group-hover:scale-105 transition-transform duration-200"
            />
            <h3 className="font-semibold text-lg text-gray-800 mb-1 truncate">{item.name}</h3>
