      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center">
            <div className="flex" aria-label={`Rating: ${products.rating} out of 5`}>
              {[1, 2, 3, 4, 5].map((star) => (
                <span key={star}>
          ...
          <p className="text-2xl font-bold text-green-600">₹{products.price?.toLocaleString('en-IN')}</p>
      ...
      <div className="mb-12">
        <div className="flex border-b border-gray-200">
          <button
            ...
            className={`py-3 px-6 font-medium ${active === 'description' ? 'text-green-600 border-b-2 border-green-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
            Description
          </button>
          <button
            onClick={() => setActive('review')}
            className={`py-3 px-6 font-medium ${active === 'review' ? 'text-green-600 border-b-2 border-green-600' : 'text-gray-500 hover:text-gray-700'}`}
          >
            Reviews
          </button>
        </div>
      ...
      <div className="mb-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {category.slice(0,5).map((item) => (
            item.id !== productId && (
              <div key={item.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden border border-gray-100">
